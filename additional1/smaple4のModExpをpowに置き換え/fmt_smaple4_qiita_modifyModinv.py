...
def Garner(a_,b_,c_,ar_,br_,cr_):
    ...
    x = x + (cr - x % c) * sym.mod_inverse(a,c) % c * sym.mod_inverse(b,c) % c * a * b
...
class FMTClass():
    ...
    # MODP剰余下でaのb乗を返す
    def ModExp(self,a, b):
        return np.uint32(pow(int(a), int(b), self.MODPint))
    ...
    def DivN(self,inputA):
        digitN_r = sym.mod_inverse(digitN, int(self.MODP))  # digitNの逆元を計算
...
if __name__=="__main__":
    ...
    E=Carrying(E0,E1,E2)
    st -= time.time()
    print("{0}\t{1}".format(digit_level,-st))
