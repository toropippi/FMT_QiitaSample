...
//aのn乗 mod c
uint expmod(uint a,uint n,uint MOD){
  ull ans=1ULL,aa=a,beki=n,ulm=(ull)MOD;
  for(int i=0;i<63;i++){
    if (beki%2==1) ans=ans*aa%ulm;
    aa=aa*aa%ulm;
    beki/=2;
    if (beki==0)break;
  }
  ...
  uint t2,t0,t1,arrayAt0,arrayAt1,w0,w1;
    for(int i=0;i<digit_level;i++){
      ...
      for(int idx=0;idx<digitN/2;idx++){
        ...
        arrayAt0 = arrayA[t0];
        arrayAt1 = arrayA[t1];
        w0 = expmod(MODP_Wn, digitN - t2 * (1 << (digit_level - 1 - i)),MODP);
...
void PreNegFMT(vector<uint>& arrayA){
  for(int i=0;i<digitN;i++){
    arrayA[i] = (uint)((ull)arrayA[i] * (ull)expmod(MODP_WnSqrt, i,MODP) % (ull)MODP);
  }
}

void PostNegFMT(vector<uint>& arrayA){
  for(int i=0;i<digitN;i++){
    arrayA[i] = (uint)((ull)arrayA[i] * (ull)expmod(MODP_WnSqrt, digitN * 2 - i,MODP) % (ull)MODP);
  }
}
